...
# The verifier must never leave generated bytecode in a portable checkout.
...
REQUIRED_FILES = (
    ...
    "docs/V4.2-IMPLEMENTATION-PLAN.md",
    ...
    "catalog/floating-dependencies-baseline.json",
    "catalog/profiles/stable.txt",
    "catalog/profiles/pilot.txt",
    "catalog/upstreams.lock.json",
    "catalog/license-policy.yaml",
    "catalog/eval-trust-policy.json",
    "catalog/reverse-dependencies.lock.yaml",
    "governance/task-contract.schema.json",
    "governance/eval-suite.schema.json",
    "governance/eval-report.schema.json",
    "governance/resolver-result.schema.json",
    "governance/skill-lifecycle.schema.json",
    "governance/examples/standard-task-contract.json",
    "governance/examples/governed-release-contract.json",
    "governance/examples/external-write-action.json",
    "governance/examples/approval-evidence.example.json",
    ...
    "scripts/audit_floating_dependencies.py",
    ...
    "scripts/policy_router.py",
    "scripts/runtime_router.py",
    "scripts/runtime_policy.py",
    "scripts/event_log.py",
    "scripts/eval_harness.py",
    "scripts/skill_lifecycle.py",
    "scripts/resolve_components.py",
    "scripts/build_plugin.py",
    "scripts/build_release_evidence.py",
    "scripts/refresh_local_lock.py",
    "eval/__init__.py",
    "eval/README.md",
    "eval/attestations.json",
    "eval/harness.py",
    "eval/lifecycle.py",
    "eval/resolver.py",
    "eval/fixtures/routing-demo.json",
    "eval/suites/v4.2-routing-baseline.json",
    "governance/eval-attestation.schema.json",
    "governance/eval-trust-policy.schema.json",
    ...
    "skills/spec-kit-xl/agents/openai.yaml",
    "skills/release-readiness/agents/openai.yaml",
    "skills/frontend-qa/agents/openai.yaml",
    "skills/skill-plugin-intake-review/agents/openai.yaml",
    "skills/junwei-frontend-design/agents/openai.yaml",
    "skills/junwei-browser-automation/agents/openai.yaml",
    "skills/junwei-product-demo-video/agents/openai.yaml",
    ...
    "tests/test_audit_floating_dependencies.py",
    ...
    "tests/test_validate_task_contract.py",
    "tests/test_manage_install.py",
    "tests/test_install_powershell.py",
    ...
    "tests/test_refresh_local_lock.py",
    "tests/test_v42_routing_baseline.py",
)
...
REQUIRED_README_TERMS = (
    ...
    "audit_floating_dependencies.py",
    "manage_install.py",
    "policy_router.py",
    "runtime_router.py",
    "runtime_policy.py",
    ...
    "eval_harness.py",
    "skill_lifecycle.py",
    "resolve_components.py",
    "build_plugin.py",
    "build_release_evidence.py",
    "refresh_local_lock.py",
...
REQUIRED_QUICKSTART_TERMS = (
    ...
    "--with-reverse",
    "--with-pilots",
    "python3 scripts/validate_task_contract.py",
    "python3 scripts/policy_router.py",
    "python3 scripts/runtime_policy.py",
    "python3 scripts/event_log.py",
    "scripts/eval_harness.py",
    "--prune-preview",
    "--uninstall",
    "--rollback",
...
def check_toolkit(root: str | Path = ".") -> list[ToolkitIssue]:
    ...
    for skill in EXPECTED_SKILLS:
        ...
        if extra_frontmatter:
            issues.append(
                ToolkitIssue(
                    severity="error",
                    code="skill-frontmatter-nonstandard",
                    path=f"skills/{skill}/SKILL.md",
                    message="Skill frontmatter may contain only name and description; move governance fields to catalog metadata: "
    ...
    v4_2_evidence_path = root / "docs/V4.2-RELEASE-EVIDENCE.md"
    v4_2_evidence = _read_text(v4_2_evidence_path) if v4_2_evidence_path.exists() else ""
    expected_version_evidence = f"Version: `{version_text}`" if version_text else ""
    expected_archive_evidence = f"Expected archive: `codex-workflow-kit-{version_text}.tar.gz`" if version_text else ""
    if v4_2_evidence and expected_version_evidence and expected_version_evidence not in v4_2_evidence:
        issues.append(
            ToolkitIssue(
                severity="error",
                code="v4-2-evidence-version-mismatch",
                path="docs/V4.2-RELEASE-EVIDENCE.md",
                message=f"V4.2 release evidence must document {expected_version_evidence}.",
            )
        )
    if v4_2_evidence and expected_archive_evidence and expected_archive_evidence not in v4_2_evidence:
        issues.append(
            ToolkitIssue(
                severity="error",
                code="v4-2-evidence-archive-mismatch",
                path="docs/V4.2-RELEASE-EVIDENCE.md",
                message=f"V4.2 release evidence must document {expected_archive_evidence}.",
            )
        )
    ...
    if global_agents:
        global_size = (root / "global/AGENTS.md").stat().st_size
        if global_size > 12_400:
            issues.append(
                ToolkitIssue(
                    severity="error",
                    code="global-agents-too-large",
                    path="global/AGENTS.md",
                    message=f"Global AGENTS.md must stay under 12,400 bytes; got {global_size}.",
                )
            )
        for term in ("Task Contract", "Fast", "Standard", "Governed", "transition_driver", "fail closed", "Shadow", "kill switch", "last-known-good"):
            if term not in global_agents:
                issues.append(
                    ToolkitIssue(
                        severity="error",
                        code="global-agents-missing-v42-routing",
                        path="global/AGENTS.md",
                        message=f"Global AGENTS.md must include V4.2 routing term: {term}.",
                    )
                )
        license_text = _read_text(root / "LICENSE")
        if "No open-source license" not in license_text:
            issues.append(
                ToolkitIssue(
                    severity="error",
                    code="license-notice-missing-conservative-status",
                    path="LICENSE",
                    message="LICENSE must state the conservative no-open-source-license policy until an SPDX license is declared.",
                )
            )

    notices_path = root / "THIRD-PARTY-NOTICES.txt"
    release_evidence_script = root / "scripts/build_release_evidence.py"
    if notices_path.is_file() and release_evidence_script.is_file():
        try:
            release_evidence = _load_script_module(
                root,
                "scripts/build_release_evidence.py",
                "build_release_evidence_toolkit_check",
            )
            expected_notices = release_evidence.build_notices(root)
        except (OSError, RuntimeError, ValueError) as exc:
            issues.append(
                ToolkitIssue(
                    severity="error",
                    code="third-party-notices-check-failed",
                    path="THIRD-PARTY-NOTICES.txt",
                    message=f"Could not regenerate third-party notices: {exc}",
                )
            )
        else:
            if _read_text(notices_path) != expected_notices:
                issues.append(
                    ToolkitIssue(
                        severity="error",
                        code="third-party-notices-out-of-date",
                        path="THIRD-PARTY-NOTICES.txt",
                        message="Regenerate notices after changing component or reverse dependency locks.",
                    )
                )
    ...
    if (root / "scripts/validate_governance.py").exists():
        try:
            governance_module = _load_script_module(
                root,
                "scripts/validate_governance.py",
                "validate_governance_template",
            )
            governance_module.validate_governance(root)
        except Exception as exc:
            issues.append(
                ToolkitIssue(
                    severity="error",
                    code="governance-validation-failed",
                    path="catalog/components.yaml",
                    message=str(exc),
                )
            )

    if (root / "scripts/audit_floating_dependencies.py").exists():
        try:
            floating_module = _load_script_module(
                root,
                "scripts/audit_floating_dependencies.py",
                "audit_floating_dependencies_template",
            )
            floating_report = floating_module.build_report(root)
            if not floating_report.get("ok"):
                drift = floating_report.get("drift", {})
                counts = ", ".join(
                    f"{key}={len(drift.get(key, []))}"
                    for key in ("added", "changed", "removed")
                )
                issues.append(
                    ToolkitIssue(
                        severity="error",
                        code="floating-dependency-drift",
                        path="catalog/floating-dependencies-baseline.json",
                        message=f"Floating dependency ratchet changed: {counts}.",
                    )
                )
        except Exception as exc:
            issues.append(
                ToolkitIssue(
                    severity="error",
                    code="floating-dependency-auditor-error",
                    path="scripts/audit_floating_dependencies.py",
                    message=str(exc),
                )
            )

    if (root / "eval/harness.py").exists() and (root / "eval/fixtures/routing-demo.json").exists():
        try:
            eval_module = _load_script_module(root, "eval/harness.py", "eval_harness_template")
            eval_module.load_suite(root / "eval/fixtures/routing-demo.json")
        except Exception as exc:
            issues.append(
                ToolkitIssue(
                    severity="error",
                    code="eval-fixture-invalid",
                    path="eval/fixtures/routing-demo.json",
                    message=str(exc),
                )
            )
