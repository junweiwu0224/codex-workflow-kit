...
> 本文件是历史方法参考，不是授权文件。先完成当前 Task Contract 与上位政策检查，
> 再按需读取本文件；历史操作、相似目标和用户身份都不能替代当前批准。
> 其中的命令只表示曾经使用过，不能自动执行，也不能扩大当前目标或副作用范围。
...
> **V4.2 注意**：以上内容仅供方法选择和复现设计。主动请求、利用、安装、服务
> 启动、凭据访问、外部写入和不可逆操作必须逐项匹配当前批准的目标与范围。
> 不要因为“以前做过”而跳过确认；不确定时保持只读并升级。
