...
- 当前上游路由已包含 Unity / IL2CPP，但仍缺一个真实存在的 `game-security/` 独立模块
- 后续如果补独立模块，建议新增 `game-security/references/il2cpp-cheatsheet.md`：dump 工具对比、frida-bridge 模板、加密 metadata 处理
...
## 进化动作
- [ ] 以真实目录形式补齐 `game-security/`，再把本种子中的 IL2CPP 完整章节迁入
