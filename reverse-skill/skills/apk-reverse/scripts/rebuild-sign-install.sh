...
find_android_build_tool() {
    ...
    local roots=(
        ...
        "$HOME/Android/Sdk"
        "$HOME/homebrew/share/android-commandlinetools"
    )
    ...
    for root in "${roots[@]}"; do
        ...
        done < <(find "$root/build-tools" -type f -name "$tool_name" | sort -Vr)
    done
    return 1
}
...
ensure_tool() {
    ...
    if [[ "$name" == "zipalign" || "$name" == "apksigner" ]]; then
        local discovered
        discovered="$(find_android_build_tool "$name" || true)"
