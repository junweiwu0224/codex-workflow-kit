...
### Free binary workflow in Codex
```bash
bash skills/reverse-engineering/scripts/free-binary-workflow.sh /bin/cat \
  --analyze \
  --rename-smoke \
  --label bin-cat-free-chain
```

这条入口会复用两段已经验证过的链路：

- 先调用 `skills/radare2/scripts/recon.sh` 做免费快速侦察
- 再从 `~/.codex/config.toml` 读取当前已注册的 `ghidra` MCP bridge，走 headless Ghidra 做 `segments / exports / xrefs / rename`

默认输出到当前工作目录下的 `.verification/reverse-skill/out/`，所以更适合从你的实际项目目录调用，而不是从 skill 包目录里调用。

安全边界：

- 默认只读；只有显式加 `--rename-smoke` 才会做一次 rename 验证
- rename 验证发生在临时 Ghidra project 中，并且会自动恢复原名
- 不依赖 IDA Pro，也不要求 GUI Ghidra 插件在线
...
- Analysis: `decompile_function`, `get_xrefs_to`, `get_xrefs_from`, `get_function_xrefs`, `list_exports`, `list_segments`
- Annotation: `rename_function`, `rename_function_by_address`, `rename_data`

### Codex headless MCP minimal chain
```text
1. open_binary
2. get_current_binary
3. list_segments
4. list_exports
5. get_xrefs_to / get_function_xrefs / get_xrefs_from
6. optional rename_function_by_address (temporary project only)
7. close_binary
```

如果你的目标是“免费替代链”而不是 IDA 主链，优先走上面的 `free-binary-workflow.sh`。当前这台机器上，这条链已经真实验证过对 `/bin/cat` 可跑通 `r2 recon + ghidra xrefs / exports / segments / rename smoke`。
