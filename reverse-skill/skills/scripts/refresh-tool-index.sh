...
# Ensure homebrew tools are findable by command -v
...
install_hint() {
  ...
  case "$PLATFORM:$name" in
    ...
    linux:jshookmcp) echo "npx: npx -y @jshookmcp/jshook@0.3.3" ;;
    ...
    macos:jshookmcp) echo "npx: npx -y @jshookmcp/jshook@0.3.3" ;;
...
TOOLS=(
  ...
  "apksigner|apk-reverse|APK signing|apksigner|apksigner --version|$HOME/Library/Android/sdk/build-tools/*/apksigner;$HOME/Android/Sdk/build-tools/*/apksigner;$HOME/homebrew/share/android-commandlinetools/build-tools/*/apksigner"
  "adb|apk-reverse|Android device bridge|adb|adb version|$HOME/tools/android-platform-tools/platform-tools/adb"
  "zipalign|apk-reverse|APK alignment|zipalign|zipalign -h|$HOME/Library/Android/sdk/build-tools/*/zipalign;$HOME/Android/Sdk/build-tools/*/zipalign;$HOME/homebrew/share/android-commandlinetools/build-tools/*/zipalign"
  ...
  "burpsuite|burp-mcp|BurpSuite desktop application|burpsuite|burpsuite --version|/Applications/Burp Suite.app;/Applications/Burp Suite Professional.app;/Applications/Burp Suite Community Edition.app"
  "graphviz|diagram-generator|Graphviz diagram rendering|dot|dot -V|"
  "plantuml|diagram-generator|PlantUML diagram rendering|none|none|$HOME/tools/plantuml/plantuml.jar"
...
for entry in "${TOOLS[@]}"; do
  ...
  if [[ "$available" == "no" && -n "${probes:-}" ]]; then
    ...
    path="$(first_existing_probe "${probe_list[@]}" || true)"
    if [[ -n "$path" ]]; then
      available="yes"
    fi
  fi
...
# Generate JSON from markdown table using the standalone script
MD_TO_JSON="$SCRIPT_DIR/md-to-json.py"
if [[ -x "$MD_TO_JSON" ]]; then
  "$MD_TO_JSON" "$OUTPUT_MD" "$OUTPUT_JSON"
else
  # Fallback: inline Python heredoc (if md-to-json.py is missing)
